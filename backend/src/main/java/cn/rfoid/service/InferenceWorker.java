...
public class InferenceWorker {
    ...
    public InferenceWorker(AppProperties config, ObjectMapper json) {
        this(config, json, List.of(config.python(), "-u", config.worker(), "--serve", "--model", config.model()), 5000);
    }
    ...
    public void warmup() throws IOException, InterruptedException { awaitReady(() -> false); }
    ...
    public Map<String, Object> status() {
        ...
        if (ready != null) for (String field : List.of("startupMs", "dependencyMs", "modelLoadMs", "warmupMs", "modelHashMs", "device", "batchSize", "imageSize", "precision")) {
