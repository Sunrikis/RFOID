...
public class ReportController {
    ...
    @GetMapping("/report") ResponseEntity<byte[]> report(@PathVariable String id) throws Exception {
        ...
        for(Map<String,Object> e:events) {
            ...
            html.append(image(id,result,e.get("snapshot").toString())).append("<p><b>处置建议：</b>").append(escape(e.get("advice"))).append("</p><p><b>处理备注：</b>").append(escape(e.get("reviewNote"))).append("</p><p class='muted'>复核时间：").append(localTime(e.get("reviewedAt"))).append("</p></section>");
        }
        html.append("<h2>模型与检测边界</h2><p>模型：").append(escape(result.get("model"))).append("；处理帧数：").append(result.get("sampledFrames")).append("；有效类别：").append(escape(result.get("capabilities"))).append("。</p><p class='muted'>自动轨道区域基于直线几何估计；弯道、道岔、遮挡或移动机位需要重新校准。抽帧可能漏掉瞬时侵入，目标关联存在重复计数风险。检测结果用于巡检辅助，应由值守人员结合现场情况复核。</p><p class='muted' style='word-break:break-all'>模型 SHA-256：").append(escape(result.get("modelSha256"))).append("</p></main></body></html>");
    ...
    @GetMapping("/events.csv") ResponseEntity<byte[]> csv(@PathVariable String id) {
        Map<String,Object> task=repo.get(id);
        StringBuilder text=new StringBuilder("\ufeff任务编号,检测模型,异物类别,置信度,风险等级,视频时间秒,检测时间,处理状态,处理备注,处置建议\r\n");
        for(Map<String,Object> e:repo.events(id,"","")) {
            Object[] values={id,task.get("modelName"),e.get("label"),e.get("confidence"),e.get("risk"),e.get("frameTime"),localTime(e.get("createdAt")),status(e.get("status")),e.get("reviewNote"),e.get("advice")};
