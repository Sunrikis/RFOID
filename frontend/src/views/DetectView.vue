...
async function loadModels() {
  ...
  try {
    ...
    if (!selectedModel.value)
      modelId.value =
        (
          models.value.find((m) => m.isDefault && m.available) ||
...
<template>
  <div class="page-heading">
    ...
    <span class="soft-badge"><ShieldCheck :size="15" />本地处理 · 自动留存</span>
  </div>
      ...
      <section class="panel upload-panel">
        ...
        <div v-else class="selected-media">
          ...
          <div v-if="isVideo" class="video-calibration-controls">
            ...
            />
            <p class="field-hint">
              拖动进度条查看静态画面。手动标定时，沿钢轨范围依次点击四个顶点；切换画面会保留标定。
            </p>
          </div>
      ...
      <section class="panel region-panel">
        ...
        <div class="region-options">
          <label :class="{ selected: mode === 'auto' }"
            ><input v-model="mode" type="radio" value="auto" :disabled="submitting" /><span
              ><b>自动识别</b><small>适合清晰、近似直线的轨道</small></span
          ...
          ><label :class="{ selected: mode === 'manual' }"
            ><input v-model="mode" type="radio" value="manual" :disabled="submitting" /><span
              ><b>手动标定钢轨范围</b
              ><small>{{
                isVideo ? '选择视频画面，依次标记四个顶点' : '在预览图上依次标记四个顶点'
              }}</small></span
        ...
        <div class="info-note">
          ...
          <p>
            {{
              mode === 'auto'
                ? '自动识别基于轨道直线特征。弯道、道岔、遮挡等复杂画面，建议人工标定并复核。'
                : '顺时针或逆时针点击四个顶点，形成不交叉的四边形。视频手动标定适用于固定机位。'
            }}
          </p>
        </div>
        <p
    ...
    <aside class="detection-side">
      <section class="panel parameter-panel">
        ...
        >
          ...
          >
            {{ model.name }}{{ !model.available ? '（未就绪）' : '' }}
          </option>
        </select>
        ...
        <select id="sampling-mode" v-model="samplingMode" class="input" :disabled="submitting">
          <option value="all">逐帧检测 · 完整检查每一帧</option>
          <option value="sampled">间隔抽帧 · 减少处理帧数</option>
        </select>
        <p v-if="samplingMode === 'all'" class="field-hint">
          {{
            isVideo
              ? '按视频原始帧率逐帧检查，GPU 自动批量处理。'
              : '图片只检测一帧；视频将逐帧检查。'
          }}
          实际处理速度在结果中显示，单任务最多 14400 帧。
        </p>
        <template v-if="samplingMode === 'sampled'">
          <div class="range-heading">
          ...
          <p class="field-hint">
            {{
              isVideo && videoDuration
                ? `视频 ${videoDuration.toFixed(1)} 秒，预计检测约 ${estimatedFrames} 帧。`
                : isVideo
                  ? '短视频按当前间隔抽帧。'
                  : '图片检测不受此项影响。'
            }}间隔越大，处理帧数越少；可能遗漏瞬时侵入。
          </p>
        </template>
        ...
        <p class="submit-hint">原始素材与检测结果将自动保存</p>
      </section>
    </aside>
  </div>
</template>
