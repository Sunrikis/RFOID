...
class InferenceRuntime:
    ...
    def acquire(self, model_path, cancelled=None):
        ...
        timings = {'modelLoadMs': 0.0, 'modelHashMs': 0.0, 'warmupMs': 0.0}
        ...
        before = time.perf_counter()
        # Cover both image and video batches before accepting jobs. All jobs keep the same precision.
        for height in sorted({self.image_size, (round(self.image_size * 9 / 16 / 32) * 32)}):
            frame = np.zeros((height, self.image_size, 3), dtype=np.uint8)
            for batch in sorted({1, self.batch_size}):
                for _ in range(2):
                    model.predict([frame] * batch, conf=.95, classes=None, **self.predict_options())
        self.synchronize()
        timings['warmupMs'] = (time.perf_counter() - before) * 1000
        check_cancelled(cancelled)
...
def run(request, output, runtime=None, cancelled=None):
    ...
    timings = {key: 0.0 for key in ('decodeMs', 'regionMs', 'regionComputeMs', 'associationMs', 'annotationMs', 'snapshotMs', 'progressMs')}
    ...
    try:
        ...
        for offset in range(0, len(frame_indices), batch_size):
            ...
            for (index,timestamp,frame), frame_boxes, region_future in zip(batch,boxes,region_futures):
                ...
                if current_roi is None:
                    raise DetectionError('RAIL_NOT_FOUND',f'Cannot establish rail region at {timestamp:.2f}s')
                before=time.perf_counter()
                h,w=frame.shape[:2]
                ...
                for values in frame_boxes:
                    ...
                    detections.append({'category':category,'label':label,'modelLabel':name,
                        ...
                        'frameTime':round(timestamp,3),'snapshot':snapshot,'advice':ADVICE[category] if danger else '目标位于危险区外，不生成预警；如轨道区域有偏差，请重新标定。'})
        ...
        accounted = sum(timings[key] for key in ('startupMs', 'decodeMs', 'regionMs', 'modelLoadMs', 'modelHashMs', 'warmupMs', 'postprocessMs', 'inferenceMs', 'progressMs'))
        timings['otherMs'] = max(0, timings['workerMs'] - accounted)
        pipeline_ms=timings['workerMs']-sum(timings[key] for key in ('startupMs', 'modelLoadMs', 'modelHashMs', 'warmupMs'))
        return {'ok':True,'detections':detections,'region':roi,'regionSource':'MANUAL' if manual else 'AUTO_GEOMETRY',
                ...
                'notes':['轨道自动识别采用直线几何估计，复杂轨道请人工校准。',
                         '视频逐帧检测并关联计数，遮挡与快速运动仍可能造成漏检或重复计数。' if sample==0 else '视频使用抽帧检测和 IoU 关联计数，快速运动或遮挡可能造成漏检或重复计数。',
                         '仅保存总览及目标证据截图；吞吐速度按实际检测帧数计算。']}
    finally:
...
def serve(default_model):
    ...
    try:
        runtime = InferenceRuntime()
        _, _, timings = runtime.acquire(default_model)
...
def main():
    ...
    parser.add_argument('--model', type=Path, help='Default model to preload for the persistent worker')
    parser.add_argument('--request',type=Path)
    parser.add_argument('--output',type=Path)
    args=parser.parse_args()
    if args.serve:
        if args.model is None or args.request is not None or args.output is not None:
            parser.error('--serve requires --model and cannot be combined with --request/--output')
