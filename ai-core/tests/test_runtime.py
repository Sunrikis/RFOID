...
from worker import DetectionError, InferenceRuntime, run, save_image
...
class RuntimeTests(unittest.TestCase):
    def setUp(self):
        ...
        options = patch.dict(os.environ, {'RFOID_BATCH_SIZE':'8', 'RFOID_IMAGE_SIZE':'640', 'RFOID_CPU_THREADS':'4', 'RFOID_HALF':'true'})
        options.start()
    ...
    def test_repeated_acquire_reuses_model_warmup_and_fingerprint(self):
        first, reused, _ = self.runtime.acquire(self.model)
        warmups = list(self.predictions)
        ...
        self.assertTrue(warmups and all(classes is None for classes in warmups))
        self.assertEqual(self.predictions, warmups)
    ...
    def test_targets_and_task_timings_are_not_carried_between_jobs(self):
        ...
        for category, expected in [('person', [0]), ('vehicle', [1]), ('motorcycle', [4]), ('obstacle', [3, 5]), ('person', [0])]:
            ...
            for field in ('startupMs', 'modelLoadMs', 'modelHashMs', 'warmupMs'):
                self.assertEqual(result['timings'][field], 0)
            parts = ['startupMs', 'modelLoadMs', 'modelHashMs', 'warmupMs', 'decodeMs', 'regionMs', 'inferenceMs', 'postprocessMs', 'progressMs', 'otherMs']
    ...
    def test_all_frames_batches_include_tail_and_keep_final_evidence(self):
        ...
        self.assertAlmostEqual(result['performance']['pipelineFps'], 6000/result['timings']['workerMs'], delta=1)
