...
def main():
    ...
    parser.add_argument('--sample-seconds', type=float, default=0, help='0 checks every frame; .5..5 samples')
    ...
    print('Loading dependencies and warming the model once...', flush=True)
    ...
    rows = []
    # An excluded full pass stabilizes codec and CUDA shape setup. Its cost is still reported.
    for index in range(args.runs+1):
        ...
        print(json.dumps({'run': 'warmup' if index==0 else index, **row}, ensure_ascii=False), flush=True)
        if index: rows.append(row)
        else: warmup = row
    import torch
    import ultralytics
    ...
    summary = dict(model=args.model.name, source=args.source.name, runs=rows, warmupRun=warmup,
